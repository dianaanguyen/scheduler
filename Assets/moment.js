...
var currentTime = $("<p>");
currentTime.text(moment().formate("h:mm a")); //store current time
...
function colorChange () { //will change color of the text area based on past, present, future
    for (var i = 9 ; i <= 17 ; i++) {
        ...
        if (currentHour > hourID) {
            ...
            textArea.css("backgroundColor", "whitesmoke"); //color for present time
        } else if (currentHour < hourID) {
            textArea.css("backgroundColor", "lightpink"); //color for future
        }
    }
}
